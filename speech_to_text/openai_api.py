...
INPUT_COST_PER_1M_TOKENS = 0.15  # 入力トークンあたりのコスト（例: $0.15/1Mトークン）
OUTPUT_COST_PER_1M_TOKENS = 0.6  # 出力トークンあたりのコスト（例: $0.6/1Mトークン)
...
class OpenAIAPI:
    ...
    def text_translation(self, new_text: str) -> str:
        ...
        cost_dollar = (self.total_input_tokens / 1_000_000) * INPUT_COST_PER_1M_TOKENS + (self.total_output_tokens / 1_000_000) * OUTPUT_COST_PER_1M_TOKENS
        cost_cents = cost_dollar * 100
        print(f"Total input tokens: {self.total_input_tokens}, Total output tokens: {self.total_output_tokens}, Elapsed time: {elapsed_time:.2f} seconds, Cost(USD): ${cost_cents:.2f} cents")
